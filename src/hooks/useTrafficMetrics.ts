...
export function useTrafficMetrics(
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<TrafficMetrics> => {
      ...
      const totalLinkClicks = metrics.reduce((s, m) => s + Number((m as any).link_clicks || 0), 0);
      const totalPostEngagement = metrics.reduce((s, m) => s + Number((m as any).post_engagement || 0), 0);
      const totalVideoViews = metrics.reduce((s, m) => s + Number((m as any).video_views || 0), 0);
      ...
      const dailyMap = new Map<string, {
        ...
        profile_visits: number; link_clicks: number; ctr: number; cpc: number; frequency: number;
        count: number;
      }>();
      for (const m of metrics) {
        ...
        const existing = dailyMap.get(key) || {
          ...
          profile_visits: 0, link_clicks: 0, ctr: 0, cpc: 0, frequency: 0, count: 0
        };
        ...
        existing.link_clicks += Number((m as any).link_clicks || 0);
      ...
      const accountMap = new Map<string, {
        spend: number; impressions: number; clicks: number; reach: number;
        profile_visits: number; link_clicks: number; post_engagement: number;
        video_views: number; ctr_sum: number; cpc_sum: number; count: number;
      }>();
      for (const m of metrics) {
        const key = m.ad_account_id;
        const existing = accountMap.get(key) || {
          spend: 0, impressions: 0, clicks: 0, reach: 0,
          profile_visits: 0, link_clicks: 0, post_engagement: 0, video_views: 0,
        ...
        existing.video_views += Number((m as any).video_views || 0);
      ...
      return {
        ...
        totalProfileVisits, totalLinkClicks, totalPostEngagement, totalVideoViews,
